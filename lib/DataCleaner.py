...
class CleanData():
        ...
        for year in self.fetched_data:
            for month in self.fetched_data[year]:
                if not self.fetched_data[year][month]:
                    ...
                    continue
                self.article_data = self.fetched_data[year][month]
    ...
    def save_cleaned_data(self, START_YEAR, END_YEAR):
        if self.cleaned_data.empty: raise Exception("There is no data to save must call on .clean_data() method before saving.")
        self.cleaned_data.to_json(f"assets/Article_Data_Cleaned_Df_({START_YEAR}-{END_YEAR}).json")
    
    def reload_cleaned_data(self, START_YEAR, END_YEAR):
        self.cleaned_data = pd.read_json(f"assets/Article_Data_Cleaned_Df_({START_YEAR}-{END_YEAR}).json", index = ['Year', 'Publication Date'])
